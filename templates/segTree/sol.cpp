...
class segTree {
  ...
  struct node {
    ...
    void modify(int l, int r, type x) {
      v += x * (r - l + 1);
      lazy += x;
    }
  ...
  template<typename R, typename... T> R get(int v, int l, int r, int ll, int rr, function<R(const R&, const R&)> f, T... t) {
    if (l >= ll && rr >= r) {
  ...
  template<typename... T> void modify(int v, int l, int r, int ll, int rr, T... t) {
    if (l >= ll && rr >= r) {
      tree[v].modify(l, r, t...);
      return;      
    }
  ...
  template<typename... T> void set_modify(int v, int l, int r, int tar, T... t) {
    if (l == r) {
      tree[v] = emp;
      tree[v].modify(l, r, t...);
      return;      
