...
int main () {
  ...
  vector<vector<int>> how((int) p.size(), vector<int>(n));
  ...
  for (auto a : how) {
    ...
    for (auto ind : ord) {
      ...
      auto bs = s.lower_bound(ind);
      debug(ind, *bs, *prev(bs));
      debug((1LL * *bs - ind) * (ind - *prev(bs)));
      debug(a[ind]);
      ans *= power(Mint(p[cc]), (1LL * *bs - ind) * (ind - *(--bs)) * a[ind]);
  ...
  for (int now = 0; now < M; ++now) {
    ...
    for (auto ind : take[now]) {
      ...
      auto bs = s.lower_bound(ind);
      ans *= power(Mint(now), (1LL * *bs - ind) * (ind - *(--bs)));
