...
class SegTree {
  ...
  struct node {
    ...
    long long sum = 0, lazy = 0, mx = 0, mn = 0;
  ...
  int spec(int v, int l, int r, int ll, int rr, int& cur) {
    debug(v, l, r, ll, rr, cur);
    if (tree[v].mn > cur) {
      return 0;
    }
    if (l >= ll && rr >= r && tree[v].sum <= cur) {
      cur -= tree[v].sum;
      return r - l + 1;
    }
    int mid = (l + r) >> 1;
    ...
    if (mid >= ll && mid < rr) {
      return spec(v + 1, l, mid, ll, rr, cur) + spec(rv, mid + 1, r, ll, rr, cur);
    }
    if (mid >= ll) {
      return spec(v + 1, l, mid, ll, rr, cur);
    }
    return spec(rv, mid + 1, r, ll, rr, cur);
...
int main () {
  ...
  while (q--) {
    ...
    --x;
    if (t == 1) {
      int ind = st.find_first(0, x, [&](auto xx) {
        return xx.mx < y; 
      });
      debug(ind);
      if (ind == -1) {
