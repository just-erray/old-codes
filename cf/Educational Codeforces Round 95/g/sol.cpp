...
class segTree {
  ...
  node unite(const node& x, const node& y) {
    ...
    res.ct = (res.mn == x.mn ? x.ct : 0) + (res.mn == y.mn ? y.ct : 0);
    return res;
  }
...
int main () {
  ...
  for (int i = 0; i < n; ++i) {
    int val = a[i];
    pos[val].push_back(i);
    int sz = (int) pos[val].size();
    debug(pos[val][sz - 2]);
    st.modify(pos[val][sz - 2] + 1, i, 1);        
    if ((int) pos[val].size() >= 4) {
      st.modify(pos[val][sz - 4] + 1, pos[val][sz - 3], -1);
    }
    if ((int) pos[val].size() >= 5) {
      st.modify(pos[val][sz - 5] + 1, pos[val][sz - 4], +1);
    }
    auto res = st.get(0, i);  
    if (res.mn == 0) {
      debug(i, res.ct);
